...
def _sync_base_from_stripe(remote, user=None, payment_confirmed=None):
    ...
    if local.stripe_subscription_id and local.stripe_subscription_id != sid: return
    status = _stripe_value(remote, "status")
    terminal = status in ("canceled", "unpaid", "incomplete_expired")
    local.subscription_plan, local.subscription_cost, local.stripe_subscription_id = "standard", 15.00, sid
    if payment_confirmed is True: local.is_active = status in ("active", "trialing")
    elif payment_confirmed is False or terminal: local.is_active = False
    local.is_cancelled = bool(_stripe_value(remote, "cancel_at_period_end", False)) or terminal
...
def _sync_plus_from_stripe(remote, user=None, payment_confirmed=None):
    metadata = _stripe_value(remote, "metadata", {}) or {}
    if _stripe_value(metadata, "purpose") != "pandaspeak_plus_addon": return
    sid = _stripe_value(remote, "id")
    if not sid: return
    if user is None:
        try: user = User.objects.get(pk=_stripe_value(metadata, "user_id"))
    ...
    if not local or not local.is_active: return
    if local.plus_stripe_subscription_id and local.plus_stripe_subscription_id != sid: return
    status = _stripe_value(remote, "status")
    terminal = status in ("canceled", "unpaid", "incomplete_expired")
    local.plus_stripe_subscription_id = sid
    if payment_confirmed is True: local.plus_is_active = status in ("active", "trialing")
    elif payment_confirmed is False or terminal: local.plus_is_active = False
    local.plus_is_cancelled = bool(_stripe_value(remote, "cancel_at_period_end", False)) or terminal
...
def _apply_reward_to_upcoming_plus_invoice(invoice):
    """Make one paid-Plus renewal free using a one-time Stripe coupon.

    This runs at invoice.created, before automatic collection. The reward is
    not consumed here; it is consumed only after invoice.paid confirms $0 due.
    """
    sid = _stripe_value(invoice, "subscription")
    if not sid or _stripe_value(invoice, "billing_reason") != "subscription_cycle": return
    remote = stripe.Subscription.retrieve(sid, api_key=_subscription_api_key())
    metadata = _stripe_value(remote, "metadata", {}) or {}
    if _stripe_value(metadata, "purpose") != "pandaspeak_plus_addon": return
    try: user = User.objects.get(pk=_stripe_value(metadata, "user_id"))
    except (User.DoesNotExist, ValueError, TypeError): return
    if not available_reward_count(user): return
    invoice_id = _stripe_value(invoice, "id")
    invoice_metadata = _stripe_value(invoice, "metadata", {}) or {}
    if _stripe_value(invoice_metadata, "pandaspeak_referral_reward") == "1": return
    ...
    stripe.Invoice.modify(invoice_id, api_key=_subscription_api_key(), discounts=[{"coupon": coupon.id}], metadata={**dict(invoice_metadata), "pandaspeak_referral_reward": "1", "pandaspeak_reward_user_id": str(user.pk)})


def _consume_reward_from_paid_invoice(invoice):
    metadata = _stripe_value(invoice, "metadata", {}) or {}
    if _stripe_value(metadata, "pandaspeak_referral_reward") != "1": return
    if (_stripe_value(invoice, "amount_due", 0) or 0) != 0: return
    try: user = User.objects.get(pk=_stripe_value(metadata, "pandaspeak_reward_user_id"))
    except (User.DoesNotExist, ValueError, TypeError): return
...
def stripe_plus_upgrade(request):
    ...
    if not local or not local.is_active or local.is_cancelled:
        messages.error(request, "An active PandaSpeak Standard annual subscription is required before upgrading."); return redirect("subscribe")
    if local.plus_is_active:
        messages.info(request, "You already have PandaSpeak Plus."); return redirect("plus_upgrade")
    if not _plus_price():
        messages.error(request, "PandaSpeak Plus billing is not configured yet."); return redirect("plus_upgrade")
    try:
        session = stripe.checkout.Session.create(api_key=_subscription_api_key(), mode="subscription", payment_method_types=["card"], customer_email=request.user.email or None, line_items=[{"price": _plus_price(), "quantity": 1}], success_url=request.build_absolute_uri("/subscription/stripe/plus-success/") + "?session_id={CHECKOUT_SESSION_ID}", cancel_url=request.build_absolute_uri("/student/plus/"), metadata={"purpose": "pandaspeak_plus_addon", "user_id": str(request.user.id)}, subscription_data={"metadata": {"purpose": "pandaspeak_plus_addon", "user_id": str(request.user.id)}})
        return redirect(session.url)
    except stripe.error.StripeError:
        messages.error(request, "Unable to start PandaSpeak Plus checkout. Your annual subscription was not changed."); return redirect("plus_upgrade")


@login_required
def stripe_plus_migration_checkout(request): return stripe_plus_upgrade(request)
@login_required
def stripe_plus_migration_success(request): return stripe_plus_success(request)
...
def stripe_plus_success(request):
    ...
    try: session = stripe.checkout.Session.retrieve(session_id, api_key=_subscription_api_key())
        ...
        messages.error(request, "We could not verify the Plus payment. Your annual subscription was not changed."); return redirect("plus_upgrade")
    metadata, sid = _stripe_value(session, "metadata", {}) or {}, _stripe_value(session, "subscription")
    if (_stripe_value(session, "mode") != "subscription" or _stripe_value(metadata, "purpose") != "pandaspeak_plus_addon" or str(_stripe_value(metadata, "user_id")) != str(request.user.id) or not sid):
        messages.error(request, "We could not safely verify this PandaSpeak Plus checkout."); return redirect("plus_upgrade")
    try:
        _sync_plus_from_stripe(stripe.Subscription.retrieve(sid, api_key=_subscription_api_key()), user=request.user, payment_confirmed=_stripe_value(session, "payment_status") == "paid")
    except stripe.error.StripeError:
        messages.error(request, "Plus payment was received but synchronization is still pending. Please do not purchase again."); return redirect("plus_upgrade")
    messages.success(request, "Welcome to PandaSpeak Plus! Your $15 annual PandaSpeak subscription remains active separately."); return redirect("plus_upgrade")
...
def stripe_subscription_success(request):
    ...
    try: session = stripe.checkout.Session.retrieve(session_id, api_key=_subscription_api_key())
        ...
        messages.error(request, "We could not verify your Stripe subscription. Please contact PandaSpeak support if you were charged."); return redirect("subscribe")
    metadata, sid = _stripe_value(session, "metadata", {}) or {}, _stripe_value(session, "subscription")
    if (_stripe_value(session, "mode") != "subscription" or _stripe_value(metadata, "purpose") != "pandaspeak_annual_subscription" or str(_stripe_value(metadata, "user_id")) != str(request.user.id) or not sid):
        messages.error(request, "We could not verify your Stripe subscription."); return redirect("subscribe")
    status = _stripe_value(session, "payment_status")
    try: _sync_base_from_stripe(stripe.Subscription.retrieve(sid, api_key=_subscription_api_key()), user=request.user, payment_confirmed=(status == "paid") if status in ("paid", "unpaid") else None)
...
def stripe_subscription_webhook(request):
    ...
    try:
        if event_type == "invoice.created":
            ...
            metadata, sid = _stripe_value(obj, "metadata", {}) or {}, _stripe_value(obj, "subscription")
            if sid:
                try: user = User.objects.get(pk=_stripe_value(metadata, "user_id"))
                ...
                if _stripe_value(metadata, "purpose") == "pandaspeak_plus_addon": _sync_plus_from_stripe(remote, user=user, payment_confirmed=confirmed)
                elif _stripe_value(metadata, "purpose") == "pandaspeak_annual_subscription": _sync_base_from_stripe(remote, user=user, payment_confirmed=confirmed)
        elif event_type in ("customer.subscription.created", "customer.subscription.updated", "customer.subscription.deleted"):
            purpose = _stripe_value(_stripe_value(obj, "metadata", {}) or {}, "purpose")
            if purpose == "pandaspeak_plus_addon": _sync_plus_from_stripe(obj)
            elif purpose == "pandaspeak_annual_subscription": _sync_base_from_stripe(obj)
            ...
            if sid:
                ...
                if purpose == "pandaspeak_plus_addon":
                    ...
                    if confirmed: _consume_reward_from_paid_invoice(obj)
                elif purpose == "pandaspeak_annual_subscription": _sync_base_from_stripe(remote, payment_confirmed=confirmed)
    except stripe.error.StripeError:
