...
def respond_group_request(request, pk, decision):
    ...
    group_request.status=decision; group_request.save(update_fields=['status','updated_at'])
...
class GroupRequestCourseCreateView(CourseCreateView):
    """Create a group class from an accepted request using the dedicated group form."""
    ...
    def get(self,request,*args,**kwargs):
        ...
        form=GroupCourseForm(initial={'title':gr.topic,'description':gr.message,'max_students':gr.desired_group_size,'initial_capacity':gr.desired_group_size})
        return render(request,self.template_name,{'form':form,'group_request':gr,'creation_mode':'group','page_title':'Create Group Class from Request'})
    def post(self,request,*args,**kwargs):
        ...
        if form.is_valid():
            ...
            gr.created_course=course; gr.save(update_fields=['created_course','updated_at'])
