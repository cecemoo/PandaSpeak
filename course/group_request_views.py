...
from .create_views import DedicatedGroupCourseForm
...
def _requested_schedule_initial(gr):
    ...
    if day_name not in weekdays:
        return {}
    try:
        student_tz = ZoneInfo(student_tz_name)
    ...
    try:
        ...
        teacher_tz_name = settings.TIME_ZONE
...
def _ensure_time_choices(form, initial):
    for field_name in ('daily_start_time','daily_end_time'):
        ...
        if value not in {choice[0] for choice in choices}:
            choices.append((value, value)); choices.sort(key=lambda item:item[0])
...
def request_group_class(request, course_pk):
    ...
    if request.method == 'POST':
        ...
        if not topic or not preferred_day or not preferred_time or not preferred_timezone:
            ...
            gr=GroupClassRequest.objects.create(student=request.user,teacher=course.teacher,source_course=course,topic=topic,level=level,preferred_times=preferred_times,desired_group_size=desired_group_size,message=message)
            Notification.objects.create(user=course.teacher,title='New Group Class Request',message=f'{request.user.get_full_name() or request.user.email} requested a group class: {topic}.',link=reverse('course:teacher_group_requests'))
...
def my_group_requests(request):
    qs=GroupClassRequest.objects.filter(student=request.user).select_related('teacher','source_course','created_course')
    return render(request,'course/my_group_requests.html',{'group_requests':qs})


@login_required
def teacher_group_requests(request):
    qs=GroupClassRequest.objects.filter(teacher=request.user).select_related('student','source_course','created_course')
    return render(request,'course/teacher_group_requests.html',{'group_requests':qs})
...
def respond_group_request(request, pk, decision):
    ...
    if gr.status!='pending':
        messages.info(request,'This request has already been reviewed.'); return redirect('course:teacher_group_requests')
    if decision not in ('accepted','declined'):
        messages.error(request,'Invalid response.'); return redirect('course:teacher_group_requests')
    ...
    if gr.student.email:
        send_mail(f'PandaSpeak - Group Class Request {decision.title()}',f'Your group class request for “{gr.topic}” was {decision}. Please sign in to PandaSpeak for details.',settings.DEFAULT_FROM_EMAIL,[gr.student.email],fail_silently=True)
...
class GroupRequestCourseCreateView(CourseCreateView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if gr.created_course_id:
            messages.info(request,'A group class has already been created from this request.'); return redirect('course:course_detail',pk=gr.created_course_id)
        schedule=_requested_schedule_initial(gr)
        initial={'title':gr.topic,'description':gr.message,'max_students':gr.desired_group_size,'initial_capacity':gr.desired_group_size,**schedule}
        form=DedicatedGroupCourseForm(initial=initial); _ensure_time_choices(form,schedule)
    ...
    def post(self,request,*args,**kwargs):
        ...
        if gr.created_course_id:
            messages.info(request,'A group class has already been created from this request.'); return redirect('course:course_detail',pk=gr.created_course_id)
        schedule=_requested_schedule_initial(gr)
        form=DedicatedGroupCourseForm(request.POST,request.FILES); _ensure_time_choices(form,schedule)
        if form.is_valid():
            course=form.save(commit=False); course.teacher=request.user; course.session_type='group'; course.save()
